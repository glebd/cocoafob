...
def make_license(private_key_string, product_code, name):
    ...
    encoded_signature = encoded_signature.decode('utf8')
...
def main():
    ...
    # test verification
    assert (verify_license(public_key_string, license, 'product',
                           'user'))  # This throws on an invalid license
    print('verification successful')

    # test rejection of invalid signature
    assert (not verify_license(public_key_string, license, 'product',
                               'WRONGUSER'))
